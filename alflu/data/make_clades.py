...
def extract_all_mutations(node, target_clade=None):
    ...
    Exclude mutations with positions less than 1 after adding 1767.
    """
    mutations = set()
            ...
            if "branch_attrs" in node and "mutations" in node["branch_attrs"]:
                ...
                if "nuc" in branch_mutations:
                    for mutation in branch_mutations["nuc"]:
                        ref_nt = mutation[0]
                        position = int(mutation[1:-1]) + 1767 # - 106 # for ToBRFV
                        alt_nt = mutation[-1]
                        if position >= 1:
...
with open("flu_seasonal_h3n2_na_12y.json", "r") as json_file:
